...
void CGLTFImporter::ImportBoneData(const std::string& FileContent)
{
    ...
    for (; NodesPosition < FileContent.size(); NodesPosition++)
    {
        ...
        else if (FileContent[NodesPosition] == '{')
        {
            ...
            Node.m_Name = "";
            Node.m_PoseTranslation = glm::vec3(0.0f);
    ...
    uint64_t SkinDataPosition = FileContent.find(SearchSkinString) + SearchSkinString.size();
    ...
    for (; SkinDataPosition < FileContent.size(); SkinDataPosition++)
    {
        ...
        else if (std::isdigit(FileContent.at(SkinDataPosition)))
        {
            InverseBindMatrixNodeString.push_back(std::string{});
            //InverseBindMatrixNodeString.at(InverseMatrixIndexCounter) += std::atoi(FileContent.at(SkinDataPosition));
            InverseBindMatrixNodeString.push_back(std::to_string(FileContent.at(SkinDataPosition)));
            SkinDataPosition++;
            while (std::isdigit(FileContent.at(SkinDataPosition)))
            {
                InverseBindMatrixNodeString.at(InverseMatrixIndexCounter) += std::to_string(FileContent.at(SkinDataPosition));
                SkinDataPosition++;
            }
    ...
    __debugbreak();
    //for (uint64_t SkinDataIt = SkinDataPosition; )
}
